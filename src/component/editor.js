...
function keydownEventHandler(evt) {
  ...
  if (keyCode === 13 && !altKey) {
    // Enter - завершить режим формул
    if (this.formulaMode && this.formulaMode.isActive()) {
      this.formulaMode.deactivate();
    }
    evt.preventDefault();
  }
  if (keyCode === 9) {
    // Tab - завершить режим формул
    if (this.formulaMode && this.formulaMode.isActive()) {
      this.formulaMode.deactivate();
    }
  }
  if (keyCode === 27) {
    // Escape - отменить режим формул
    if (this.formulaMode && this.formulaMode.isActive()) {
      this.formulaMode.deactivate();
      // Очищаем текст
      this.setText('');
...
function inputEventHandler(evt) {
  ...
  // Проверяем активацию режима формул
  if (v === '=' && this.formulaMode && !this.formulaMode.isActive()) {
    this.formulaMode.activate();
  }
  
  // Если режим формул активен, обновляем формулу
  if (this.formulaMode && this.formulaMode.isActive()) {
    this.formulaMode.setCurrentFormula(v);
  }
...
export default class Editor {
  constructor(formulas, viewFn, rowHeight) {
    ...
    this.inputText = '';
    this.change = () => {};
    this.formulaMode = null; // Будет установлен из Sheet
  }
