...
class Table {
  ...
  render() {
    ...
    if (viewRange.sri == -1) viewRange.sri = 0
    if (viewRange.sci == -1) viewRange.sci = 0
    ...
    if (fri >= 0 || fci >= 0) {
      ...
      if (fri >= 0) {
        ...
        vr.sri = 0;
        vr.eri = fri > 0 ? fri - 1 : -1;  // Only freeze rows BEFORE the selected row
        vr.h = ty;
      ...
      // 3
      if (fci > 0) {  // Only freeze columns if freeze column is > 0
        const vr = viewRange.clone();
        vr.sci = 0;
        vr.eci = fci - 1;  // Only freeze columns BEFORE the selected column
        vr.w = tx;
