...
function getCellRowByY(y, scrollOffsety) {
  ...
  // When filtering is active, limit rows to actual data range
  let maxRowToCheck = rows.len;
  if (this.autoFilter.active() && frset.size > 0) {
    const [maxDataRow] = rows.maxCell();
    // Find the last visible row that contains data
    let lastVisibleDataRow = -1;
    for (let i = 0; i <= maxDataRow; i += 1) {
      if (!frset.has(i)) {
        lastVisibleDataRow = i;
      }
    }
    // Only check rows up to the last data row (no clicks on empty rows after)
    if (lastVisibleDataRow >= 0) {
      maxRowToCheck = lastVisibleDataRow + 1;
    }
  }
...
export default class DataProxy {
  constructor(name, rows, cols, settings) {
    ...
    this.autoFilter = new AutoFilter(rows, cols);

    // Модификация: делаем так, чтобы скрытые строки имели высоту 0.
    // Это самый простой способ заставить всю систему (скролл, отрисовку)
    // корректно работать с фильтром.
    const { rows: currentRows } = this;
    const oldGetHeight = currentRows.getHeight.bind(currentRows);
    currentRows.getHeight = (ri) => {
      if (this.exceptRowSet.has(ri)) return 0;
      return oldGetHeight(ri);
    };
    ...
    // gpt fix: Отслеживание текущей редактируемой ячейки для истории
  ...
  undo() {
    this.history.undo(this.getData(), (d, changeRange) => {
      this.setData(d);
  ...
  redo() {
    this.history.redo(this.getData(), (d, changeRange) => {
      this.setData(d);
      this.lastChangeRange = changeRange;
    });
  }
  ...
  pasteFromSystemClipboard(resetSheet, eventTrigger) {
    const { selector } = this;
    let data = {
      startRow: selector.ri,
      endRow: 0,
      startColumn: selector.ci,
      endColumn: 0
    }
    navigator.clipboard.readText().then((content) => {
      // gpt
      // Добавляем одну запись в историю перед началом групповой операции
      // this.history.add(this.getData());

      // С правками от ИИ: добавляем метаданные о диапазоне изменений
      const contentToPaste = this.parseClipboardContent(content);
      const changeRange = {
        sri: selector.ri,
        sci: selector.ci,
        eri: selector.ri + contentToPaste.length - 1,
        eci: selector.ci + (contentToPaste[0]?.length || 1) - 1
      };
      this.history.add(this.getData(), changeRange);

      let startRow = selector.ri;
      contentToPaste.forEach((row) => {
        let startColumn = selector.ci;
        row.forEach((cellContent) => {
          // this.setCellText(startRow, startColumn, cellContent, 'input'); // до gpt
          //gpt
          // Вызываем setCellText с флагом, чтобы не добавлять в историю каждую ячейку
          this.setCellText(startRow, startColumn, cellContent, 'input', true);
          startColumn += 1;
        });
        startRow += 1;
        data.endColumn = startColumn - 1;
      });
      // до gpt
      // resetSheet();
      data.endRow = startRow - 1;
      //start gtp
      // Вызываем change один раз после завершения всей операции
  ...
  parseClipboardContent(clipboardContent) {
    // Определяем разделитель строк
    const lineSeparator = /\r\n/.test(clipboardContent) ? '\r\n' : '\n';

    // Удаляем переносы строк в конце текста
    const cleanContent = clipboardContent.replace(/[\r\n]+$/, '');

    // Заменяем все переносы строк внутри кавычек на пробелы
    let processedContent = cleanContent;
    const quotedRegex = /"([^"]*)"/g;

    // Находим текст в кавычках и заменяем в нем переносы строк на пробелы
    processedContent = processedContent.replace(quotedRegex, (match) => {
      return match.replace(/[\r\n]+/g, ' ');
    });

    // Схлопываем ВСЕ группы из 2 и более пробелов в один,
    // чтобы лишние пробелы не воспринимались как разделители
    processedContent = processedContent.replace(/ {2,}/g, ' ');
    // Разбиваем на строки и ячейки
    const rows = processedContent.split(lineSeparator)
      .map(line => line.split('\t')
        .map(cell => cell.replace(/^"(.*)"$/, '$1'))); // Удаляем внешние кавычки

    return rows;
  }

  pasteFromText(txt) {
    // Удаляем символы переноса строки в конце текста
  ...
  setCellText(ri, ci, text, state, skipHistory = false) {
    const { rows, history, validations } = this;

    // gpt fix: Сохраняем состояние только при начале редактирования (state='input') и только один раз
    if (!skipHistory && state === 'input') {
      const cellKey = `${ri}_${ci}`;
      // gpt fix: Проверяем, начинаем ли мы редактирование новой ячейки
      if (!this.currentEditingCell || this.currentEditingCell !== cellKey) {
        history.add(this.getData(), { sri: ri, sci: ci, eri: ri, eci: ci });
        this.currentEditingCell = cellKey;
      }
    } else if (state === 'finished') {
      // gpt fix: При завершении редактирования сбрасываем флаг
      this.currentEditingCell = null;
    }
    ...
    if (state === 'finished') {
      ...
      rows.setCellText(ri, ci, text);
      // start gpt
      // Вызываем change только если не установлен флаг пропуска
      if (!skipHistory) {
        this.change(this.getData());
      }
      // end gpt
  ...
  // Get filter history info
  getFilterHistoryInfo() {
    const { autoFilter } = this;
    return autoFilter.getHistoryInfo();
  }
  ...
  resetAutoFilter() {
    ...
    // Reset vertical scroll to top after filtering to show filtered results
    // Keep horizontal scroll position unchanged
    this.scroll.y = 0;
    this.scroll.ri = 0;
  }
  ...
  scrolly(y, cb) {
    const {
      scroll, freeze, rows, exceptRowSet,
    } = this;
    const [fri] = freeze;
    const [
      ri, top, height,
    ] = helper.rangeReduceIf(fri, rows.len, 0, 0, y, i => (exceptRowSet.has(i) ? 0 : rows.getHeight(i)));
    let y1 = top;
    if (y > 0) y1 += height;
  ...
  freezeTotalHeight() {
    return this.rows.sumHeight(0, this.freeze[0], this.exceptRowSet);
  }
  ...
  freezeViewRange() {
    const [ri, ci] = this.freeze;
    // Frozen area should only include rows/columns BEFORE the selected cell
    const endRi = ri > 0 ? ri - 1 : -1;
    const endCi = ci > 0 ? ci - 1 : -1;
    return new CellRange(0, 0, endRi, endCi, this.freezeTotalWidth(), this.freezeTotalHeight());
  }

  contentRange() {
    const {
      rows, cols, exceptRowSet,
    } = this;
    const [ri, ci] = rows.maxCell();
    const h = rows.sumHeight(0, ri + 1, exceptRowSet);
  ...
  viewRange() {
    ...
    // When filtering is active, limit rows to actual data range
    let maxRowToShow = rows.len;
    if (this.autoFilter.active() && exceptRowSet.size > 0) {
      const [maxDataRow] = rows.maxCell();
      // Find the last visible row that contains data
      let lastVisibleDataRow = -1;
      for (let i = 0; i <= maxDataRow; i += 1) {
      ...
      // Only show rows up to the last data row (no empty rows after)
      if (lastVisibleDataRow >= 0) {
        maxRowToShow = lastVisibleDataRow + 1;
      }
    }
    
    for (let i = ri; i < Math.min(rows.len, maxRowToShow); i += 1) {
      if (!exceptRowSet.has(i)) {
        y += rows.getHeight(i);
        eri = i;
      }
  ...
  rowEach(min, max, cb) {
    ...
    const { rows } = this;
    const frset = this.exceptRowSet;
    // The min, max are data row indices from viewRange
    for (let i = min; i <= max; i += 1) {
      // if row is not in the filtered set
      if (!frset.has(i)) {
